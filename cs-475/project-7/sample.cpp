...
const char *WINDOWTITLE = {"OpenCL/OpenGL Particle System -- Bryce Graves"};
...
void InitLists() {
  ...
  glColor3f(.9f, .9f, 0.);  // 0. <= r,g,b <= 1.
  glPushMatrix();
  glTranslatef(150.f, -800.f, 0.f);
  glutWireSphere(100.f, 100, 100);
  glPopMatrix();

  glColor3f(.9f, .9f, 0.);  // 0. <= r,g,b <= 1.
  glPushMatrix();
  glTranslatef(-250.f, -800.f, 0.f);
  glutWireSphere(100.f, 100, 100);
  glPopMatrix();
...
void SelectOpenclDevice() {
  ...
  for (int p = 0; p < (int)numPlatforms; p++) {
    ...
    for (int d = 0; d < (int)numDevices; d++) {
      ...
      {
        if (deviceType == CL_DEVICE_TYPE_CPU)  // holding a cpu already --
