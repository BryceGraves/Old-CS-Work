...
__global__ void MonteCarlo(float *Xcs, float *Ycs, float *Rs, int *Hits) {
  ...
  float tn = tanf((float)((M_PI / 180.0) * 30.0));
  Hits[gid] = 0;

  // solve for the intersection using the quadratic formula:

  float a = 1.0 + tn * tn;
  float b = -2.0 * (xc + yc * tn);
  float c = xc * xc + yc * yc - r * r;
  float d = b * b - 4.0 * a * c;

  // cascading if-statements:
  // if you used "continue;" in project #1, change to this style
  // because, if there is no for-loop, then there is nowhere to
  // continue to

  if (d > 0.0) {
    d = sqrt(d);
    float t1 = (-b + d) / (2.0 * a);  // time to intersect the circle
    float t2 = (-b - d) / (2.0 * a);  // time to intersect the circle
