...
int main(int argc, char *argv[]) {
  ...
  PrintCLError(status, "Prefix: ", stderr);
  ...
  for (int i = 0; i < (int)numPlatforms; i++) {
    ...
    for (int j = 0; j < (int)numDevices; j++) {
      ...
      clGetDeviceInfo(devices[j], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(ui), &ui,
                      NULL);
      ...
      clGetDeviceInfo(devices[j], CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(sizes),
                      sizes, NULL);
      fprintf(stderr, "\t\tDevice Maximum Work Item Sizes = %d x %d x %d\n",
              sizes[0], sizes[1], sizes[2]);

      clGetDeviceInfo(devices[j], CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size),
                      &size, NULL);
      fprintf(stderr, "\t\tDevice Maximum Work Group Size = %d\n", size);

      clGetDeviceInfo(devices[j], CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(ui),
                      &ui, NULL);
  ...
  clGetDeviceInfo(devices[0], CL_DEVICE_EXTENSIONS, extensionSize, extensions,
                  NULL);
  fprintf(stderr, "\nDevice Extensions:\n");
