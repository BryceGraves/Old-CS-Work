...
scheduler(void)
{
  struct proc *p;
  struct cpu *c = mycpu();
  uint nice_sum, magic_number, nice_total;
  c->proc = 0;
  ...
  for(;;){
    ...
    if (nice_total == 0) { continue; }

    magic_number = rand() % nice_total;
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(nice_sum < magic_number) { continue; }
...
int cps(void) {
  ...
  cprintf("pid\tppid\tname\tstate\tsize\tstart time\t\tticks\tsched\tnice\t\n");
...
int renice(int pid, int nice) {
  ...
  if (nice < MIN_NICE_VALUE || nice > MAX_NICE_VALUE) { return 1; }
