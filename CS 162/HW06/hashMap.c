...
{
	int i = 0;
	struct hashMap *biggerTable;
	struct hashMap *smallerTable;
	struct hashLink *currLink;

	biggerTable = createMap(newTableSize);
	smallerTable = ht;

	while (i < smallerTable->tableSize) {
		currLink = smallerTable->table[i];
		while (currLink != 0) {
			insertMap(biggerTable, currLink->key, currLink->value);
			currLink = currLink->next;
		}
		i++;
	}

	_freeMap(smallerTable);
	ht->table = biggerTable->table;
	ht->tableSize = biggerTable->tableSize;
	ht->count = biggerTable->count;
	free(smallerTable);
}
...
void insertMap(struct hashMap * ht, KeyType k, ValueType v)
{
	...
	//TODO: go back and add prime number function and use it here for size.
