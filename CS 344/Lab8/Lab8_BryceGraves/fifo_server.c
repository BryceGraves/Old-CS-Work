...
void remove_server_pipe(void) {
  if (isConnectionListener == TRUE) {
    ...
    fprintf(stderr, "  exit handler called on server: %d\n", getpid());
...
void handle_client(pid_t clientPid) {
  ...
  for ( ; ; ) {
    ...
    if (strcmp(buffer, CMD_EXIT) == 0) {
      ...
      int dFd = open(clientFifoData, O_RDONLY);
      int file = open(fileName, O_TRUNC | O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP);
