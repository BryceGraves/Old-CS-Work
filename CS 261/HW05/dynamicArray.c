...
void addHeap(DynArr *heap, TYPE val)
{
	...
	child = sizeDynArr(heap) - 1;

	if(_smallerIndexHeap(heap, child, parent) == child) {
...
{
	for (int max = ((heap->size / 2) - 1); max >= 0; max--) {
		_adjustHeap(heap, heap->size, max);
	}
}
/*p
...
void sortHeap(DynArr *heap)
{
	...
	for (int pos = heap->size - 1; pos >= 0; pos--) {
		swapDynArr(heap, 0, pos);
