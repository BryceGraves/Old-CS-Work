...
If you expecience problems with this script, please let me know. Send me email
...
function testCorrectPrimes {
    ...
    TEST_COUNT=0
    CORE_COUNT=0
    LINE_COUNT=$(zcat ${PRIMES}.txt.gz | wc -l)
    DIFF=zdiff
    > coreDump.log
    ...
    do
	    ...
	    ${DIFF} -q -w TestPrimes${PRIMES}_${T}.txt.gz Primes${PRIMES}.txt.gz > /dev/null
	    RES=$?
	    if [ ${RES} -ne 0 ]
	    then
	        ((ERR_COUNT++))
	        ERR_LINE_COUNT=$(${DIFF} -w TestPrimes${PRIMES}_${T}.txt.gz Primes${PRIMES}.txt.gz | grep -e "^<\|^>" | wc -l)
	        echo "    *** error on ${PRIMES}  threads = ${T}   ${ERR_LINE_COUNT}  of  ${LINE_COUNT}"
	        if [ ${DO_CLEANUP} -ne 1 ]
	        then
		        echo "        ${DIFF} TestPrimes${PRIMES}_${T}.txt.gz Primes${PRIMES}.txt.gz"
	        fi
	    fi
    done
...
function testLeakyPrimes {
    ...
    echo "  The PTheads library occosionally leaves some stuff allocated."
    echo "  Look at the leakLog.err file for results. Seeing leaks associated"
    echo "  with pthread_cancel_init is not a concern, but do look."
...
function timePrimes {
    ...
    for T in 1 2 3 4 5 7 10 14 18 25 30 35 40
    do
	    echo "  ${PRIMES} ${T}"
	    ((TEST_COUNT++))
...
PROG_BASE=primesMT
PROG=./${PROG_BASE}
#TEST_DIR=~chaneyr/Classes/cs344/Labs/Primes
TEST_DIR=/home/bryce/Code/Homework/CS 344/Lab8
SHORT_CORRECT=0
DO_CLEANUP=1
DO_LEAKS=1
DO_TIME=1
TWENTY_THREAD_TO=60
OWNER=jesse.chaney@osucascades.edu

while getopts "hu:p:cCTLx" opt
do
    case $opt in
	    h)
	        showHelp
